...
func _unhandled_input(event)->void:
	...
	if event is InputEventMouseButton and ! event.pressed:
		...
		if( state.astar_grid.is_in_boundsv( Vector2i(get_global_mouse_position() ) / 16) ):
			player_path = state.astar_grid.get_point_path( Vector2i(self.get('position').x , self.get('position').y) / state.GRID_CELL_SIZE, Vector2i( get_global_mouse_position() ) / 16)
			if player_path.size():
			...
			for point in player_path:
				...
				state.world.set_cell(1,next_move,1,Vector2(36,12))
